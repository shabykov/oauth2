...
def ping_view(request):
    return Response(data={'message': 'Auth server is available'}, status=200)
...
def get_access_token(request):
    ...
    if not request.user.is_anonymous:
        try:
            return Response(data=AccessTokenSerializer(model.objects.get(token=request.auth)).data, status=200,
                            content_type="application/json; charset=utf-8")
